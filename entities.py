...
import random
from globals import unique_object_counter  # Hinweis: Für den Zugriff auf den globalen Zähler verwenden wir Funktionen in simulation.py, hier wird der Zähler in den Methoden aktualisiert.
from globals import mutation_loss_rate

# Funktionen für Farbgebung und Kollisionsprüfung
...
class Beute:
    def __init__(self, canvas, x, y, size=20, generation=1, genome=None):
        from globals import unique_object_counter  # Importieren wir dynamisch, da der Zähler mutable ist
        self.canvas = canvas
        self.size = size
        self.x = x
        self.y = y
        ...
        self.alive = True
        self.immune = False  # Wird True, wenn durch Kooperation vergrößert
        self.stripe_id = None  # Für "Schneller Metabolismus"
        self.starving = False
        self.starve_job = None
        self.blinking = False

        # Erhöhe den globalen Zähler
        from globals import unique_object_counter as uc
        uc += 1  # (Hier müsstest du den Zähler in globals über eine Funktion erhöhen, da direkte Änderung nicht wirksam ist.)
        self.obj_id = f"{self.generation}-{uc}"
        ...
        # Tooltip-Bindings (nur im Pausenmodus – siehe simulation.py)
    ...
    def schedule_duplication(self):
        ...
        if self.immune or paused:
            return
        # Objekte, denen "essen" fehlt, dürfen sich nicht duplizieren, außer sie haben "jagen"
    ...
    def duplicate(self):
        ...
        if paused:
            ...
            return
        # Wenn "essen" fehlt und auch nicht "jagen", darf nicht dupliziert werden
        if "essen" not in self.genome and "jagen" not in self.genome:
            return
        ...
        from entities import Beute  # Selbstimport in einem Modul kann via relative Imports gelöst werden
        neue_beute = Beute(self.canvas, self.x, self.y, self.size,
                           generation=self.generation+1, genome=child_genome)
        from globals import beuten
        beuten.append(neue_beute)
        self.schedule_duplication()

    def show_tooltip(self, event):
        if not __import__("globals").globals().get("paused"):
            return
...
class Jaeger:
    ...
    def duplicate(self):
        ...
        from entities import Jaeger
        neue_jaeger = Jaeger(self.canvas, self.x, self.y, self.size,
                             generation=self.generation+1, genome=child_genome)
        neue_jaeger.genome = child_genome
        if "Angriff" in neue_jaeger.genome and neue_jaeger.genome["Angriff"] == "Killer":
            neue_jaeger.color = "#ff0000"
            neue_jaeger.canvas.itemconfigure(neue_jaeger.oval_id, fill=neue_jaeger.color)
        from globals import jaegers
        jaegers.append(neue_jaeger)
        self.fressen_count = 0
        self.duplication_scheduled = False
        self.stop_blinking()
